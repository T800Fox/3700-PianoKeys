	...
	always_ff @(posedge clk) begin : state_transition_logic
		if (rst) begin
			...
			case (current_state)
				X1: begin
					case (reaction_quality)
						QUALITY_PERFECT: begin
							...
							if (c_consecutive_perfect == CONSEC_PERFECT_TO_2) begin
								current_state <= X2;
								c_consecutive_perfect <= 0;
							end
						end
				...
				X2: begin
					case (reaction_quality)
						QUALITY_PERFECT: begin
							...
							if (c_consecutive_perfect <= CONSEC_PERFECT_TO_4) begin
								current_state <= X4;
								c_consecutive_perfect <= 0;
							end
						end
				...
				X4: begin
					c_consecutive_perfect <= 0;
					if (reaction_quality == QUALITY_POOR || reaction_quality == QUALITY_BAD) current_state <= X1;
				end

				default: begin
					current_state <= X1;
					c_consecutive_perfect <= 1;
				end
			endcase
		end
	end
